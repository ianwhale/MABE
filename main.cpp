...
//		cout << endl;
	...
	if (Global::modePL->lookup() == "run") {
		...
		while (!groups[defaultGroup]->archivist->finished) {
			...
			world->evaluate(groups[defaultGroup], AbstractWorld::groupEvaluationPL->lookup(), false, true, AbstractWorld::debugPL->lookup());  // evaluate each organism in the population using a World
